...
func (api *API) Postimage(writer http.ResponseWriter, req *http.Request) {
	...
	Bearer := ChekToken()
	request, err := http.NewRequest("POST", "http://localhost:8085/api/v2/images", bytes.NewBuffer(byteArr))
	if err != nil {
		msg := Message{
			StatusCode: 500,
			Message:    "We have some troubles. Try again",
...
func (api *API) Updateimages(writer http.ResponseWriter, req *http.Request) {
	var image models.Images
	err := json.NewDecoder(req.Body).Decode(&image)
	if err != nil {
		api.logger.Info("Invalid json recieved from client")
	...
	Bearer := ChekToken()
	request, err := http.NewRequest("PUT", "http://localhost:8085/api/v2/images/"+mux.Vars(req)["id"], bytes.NewBuffer(byteArr))
	if err != nil {
		msg := Message{
			StatusCode: 500,
			Message:    "We have some troubles. Try again",
...
func (api *API) Deleteimages(writer http.ResponseWriter, req *http.Request) {
	Bearer := ChekToken()
	request, err := http.NewRequest("DELETE", "http://localhost:8085/api/v2/images/"+mux.Vars(req)["id"], nil)
	if err != nil {
		msg := Message{
			StatusCode: 500,
			Message:    "We have some troubles. Try again",
